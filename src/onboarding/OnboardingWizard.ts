...
export async function runOnboardingWizard(
    ...
    if (welcome !== "Get Started") {
        return { completed: false, ideType, folderStructure: FolderStructure.AutoDetect };
    }
    ...
    const recommendations: Record<FolderStructure, string> = {
        [FolderStructure.AutoDetect]:
            "Will scan `.agent/skills`, `.cursor/rules`, `.cursor/skills` and any configured custom paths.",
    ...
    const confirm = await vscode.window.showInformationMessage(
        `Recommendation for **${folderStructure}**: ${recommendations[folderStructure]} Ready to finish setup?`,
        { modal: true },
        "Finish",
        "Back"
    );
